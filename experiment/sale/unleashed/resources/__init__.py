...
class MetaResource(type):
    ...
    def convert_fields(cls):
        ...
        for attr_name in cls.__resourcefields__.keys():
    ...
    def convert_embedded_resources(cls):
        ...
        for attr_name in cls.__embeddedresources__.keys():
            def getter(attr_name):
                def getter(cls):
...
class UnleashedResource(object, metaclass=MetaResource):
    __endpoint__ = None
    ...
    def to_dict(self):
        ...
        for name, field in self.__resourcefields__.items():
            out.update({name: field.to_dict()})

        for name, resource in self.__embeddedresources__.items():
            out.update({name: resource.to_dict()})

        # Reduce dict to a single null if all it's values are None
        if all([v is None for v in iter(out.values())]):
            out = None
...
class OldUnleashedResource(object):
    ...
    def from_dict(self, dict_val):
        ...
        for field_name, val in dict_val.items():
